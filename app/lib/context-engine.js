...
export function compileUserPrompt(mode, text, instruction) {
    switch (mode) {
        ...
        case 'typeset':
            return instruction
                ? `请按以下要求进行中文排版与标点规范化，并直接输出优化后的正文：\n要求：${instruction}\n\n原文：\n「${text}」`
                : `请对以下文本做中文排版/标点规范化：修正标点误用、统一引号/省略号/破折号、去除多余空格；不改写、不增删内容；保持段落与换行不变；只输出优化后的正文。\n\n「${text}」`;
        case 'proofread':
            return instruction
                ? `请按以下要求校对并直接输出修订后的正文：\n要求：${instruction}\n\n原文：\n「${text}」`
                : `请校对以下文本：只修正错别字、标点和明显语病；不改写、不增删内容；保持段落与换行不变；只输出修订后的正文。\n\n「${text}」`;
        case 'expand':
...
- 保持叙事视角一致`;
        ...
        case 'typeset':
            return `对选中文本做“中文排版与标点规范化”，不改写内容。
要求：
- 只修正错别字与标点误用（如：中英文标点混用、缺失/多余标点）
- 统一引号为“”，省略号为……，破折号为——
- 去除多余空格（不改变段落与换行）
- 不改写、不增删内容，不改变原意与语气
- 只输出优化后的正文，不要解释、不加标题、不加列表、不加引号/代码块`;

        case 'proofread':
            return `对选中文本进行校对（错别字/漏字/多字/标点/明显语病），不改写内容。
要求：
- 仅修正错别字、漏字、多字、重复字、标点误用、明显语病
- 不改写、不增删内容，不替换写作风格，不改变原意与语气
- 保持段落与换行不变
- 只输出修订后的正文，不要解释、不加标题、不加列表、不加引号/代码块`;
...
- 对话节奏要自然，长短句交替`;
        ...
        case 'outline':
            return `为“当前章节”生成写作大纲（粗纲 + 细纲）。
要求：
- 严格基于上述设定集、剧情大纲、前文回顾与当前写作位置（若本章已有正文，必须与正文一致）
- 输出两部分：粗纲、细纲
- 粗纲：3-8条要点，突出冲突/转折/节奏点/情绪走向/结尾钩子
- 细纲：按场景/段落列出推进，每条尽量包含：场景、出场人物、目标/冲突、关键动作、伏笔信息、情绪变化、章节结尾
- 不要输出正文，不要输出设定操作块
- 只输出严格 JSON：{"rough":"...","detailed":"..."}`;
